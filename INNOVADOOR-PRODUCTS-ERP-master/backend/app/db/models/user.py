...
class ProductionPaper(Base):
    ...
    # Raw material order status (managed only by Raw Material Checker): pending, issued, progress, received
    raw_material_order_status = Column(String, default="pending", nullable=False, index=True)
    ...
    shutter_items = relationship("ProductionShutterItem", back_populates="production_paper", cascade="all, delete-orphan")
    rm_shutter_items = relationship("RawMaterialShutterItem", back_populates="production_paper", cascade="all, delete-orphan")



class ProductionShutterItem(Base):
    __tablename__ = "production_shutter_items"
    ...
    ro_width = Column(String, nullable=True)
    ro_height = Column(String, nullable=True)
    thickness = Column(String, nullable=True)
    quantity = Column(Integer, nullable=True)
    sq_ft = Column(Float, nullable=True)
    sq_meter = Column(Float, nullable=True)
    laminate_sheets = Column(Float, nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())

    # Relationship
    production_paper = relationship("ProductionPaper", back_populates="shutter_items")


class RawMaterialShutterItem(Base):
    __tablename__ = "raw_material_shutter_items"

    id = Column(Integer, primary_key=True, index=True)
    production_paper_id = Column(Integer, ForeignKey("production_papers.id"), nullable=False, index=True)
    sr_no = Column(String, nullable=True)  # Serial number
    ro_width = Column(String, nullable=True)
    ro_height = Column(String, nullable=True)
    bldg_wings = Column(String, nullable=True)  # Building/Wings
    quantity = Column(Integer, nullable=True)
    sq_ft = Column(Float, nullable=True)
    sq_meter = Column(Float, nullable=True)
    laminate_sheets = Column(Float, nullable=True)
    ...
    # Relationship
    production_paper = relationship("ProductionPaper", back_populates="rm_shutter_items")



class ProductionDocsSettings(Base):
    """
    System-level settings for the Production Documentation Management System.
    Currently used to control automatic Raw Material Paper generation.
    """
    __tablename__ = "production_docs_settings"

    id = Column(Integer, primary_key=True, index=True)
    auto_generate_rm_frame = Column(Boolean, default=False, nullable=False)
    auto_generate_rm_shutter = Column(Boolean, default=False, nullable=False)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), onupdate=func.now())
