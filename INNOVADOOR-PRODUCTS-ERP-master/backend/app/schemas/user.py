...
class ProductionPaperBase(BaseModel):
    ...
    raw_material_order_status: Optional[str] = "pending"  # pending, issued, progress, received (Raw Material Checker only); None coerced to "pending"
...
class ProductionPaper(ProductionPaperBase):
    ...
    shutter_items: Optional[List['RMShutterItem']] = None
    rm_shutter_items: Optional[List['RMShutterItem']] = None
    selected_items_data: Optional[Any] = None  # Resolved full item list for selected_measurement_items; not stored in DB
    items_total_quantity: Optional[int] = None  # Sum of item quantities (rm_shutter_items or shutter_items) for list display
...
RAW_MATERIAL_ORDER_STATUSES = ("pending", "issued", "progress", "received")


class RawMaterialOrderStatusUpdate(BaseModel):
    """Request body for updating raw material order status (Raw Material Checker only)."""
    raw_material_order_status: str  # pending | issued | progress | received

    @field_validator("raw_material_order_status")
    @classmethod
    def validate_status(cls, v: str) -> str:
        if v not in RAW_MATERIAL_ORDER_STATUSES:
            raise ValueError(f"Status must be one of: {RAW_MATERIAL_ORDER_STATUSES}")
        return v


class ProductionDocsSettingsBase(BaseModel):
    auto_generate_rm_frame: bool = False
    auto_generate_rm_shutter: bool = False


class ProductionDocsSettingsResponse(ProductionDocsSettingsBase):
    id: int
...
class ProductionDocsSettingsUpdate(ProductionDocsSettingsBase):
    pass


class RMShutterItem(BaseModel):
    id: Optional[int] = None
    sr_no: Optional[str] = None
    ro_width: Optional[str] = None
    ro_height: Optional[str] = None
    bldg_wings: Optional[str] = None
    quantity: Optional[int] = None
    sq_ft: Optional[float] = None
    sq_meter: Optional[float] = None
    laminate_sheets: Optional[float] = None
    thickness: Optional[str] = None
...
class RawMaterialTableRequest(BaseModel):
    """Request model for extracting raw material table from production paper"""
    production_paper_id: Optional[int] = None  # Optional, can use path parameter instead
    overwrite_existing: Optional[bool] = False  # If True, delete existing items before creating new ones


class RawMaterialTableResponse(BaseModel):
    """Response model for raw material table extraction"""
    production_paper_id: int
    items: List[RMShutterItem]
    totals: Dict[str, Any]  # {"total_qty": int, "total_sq_ft": float}
    message: str


class RMGenerationResponse(BaseModel):
    message: str
    paper_number: str
