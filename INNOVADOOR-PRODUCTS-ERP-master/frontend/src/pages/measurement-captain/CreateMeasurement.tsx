...
import { Plus, ArrowRight, Search, ChevronDown, X, AlertTriangle, Upload, File as FileIcon, Image, FileText, FileSpreadsheet, Trash2, Eye, Mic, Square, Volume2 } from 'lucide-react';
...
export default function CreateMeasurement() {
  ...
  const getFieldsForType = (type: MeasurementType): string[] => {
    switch (type) {
      ...
      case 'shutter_sample':
        return ['sr_no', 'location', 'bldg', 'flat_no', 'area', 'width', 'height', 'minus_width', 'minus_height', 'act_width', 'act_height', 'ro_width', 'ro_height', 'remark'];
      case 'regular_frame':
        return ['sr_no', 'location_of_fitting', 'bldg', 'flat_no', 'area', 'act_width', 'act_height', 'wall', 'subframe_side', 'remark'];
      ...
      setTimeout(() => {
        ...
        if (firstEditableField) {
          ...
          for (let i = 0; i < inputs.length; i++) {
            if ((inputs[i] as HTMLElement).offsetParent !== null) {
  ...
  const handleConfirmNavigation = () => {
    ...
    navigate('/measurement-captain/measurements');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Ensure each item has an auto-generated, unique Sr No
  ...
  return (
    <>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-6xl w-full max-h-[95vh] sm:max-h-[90vh] flex flex-col mx-2" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex-1 overflow-auto p-2 sm:p-4">
              {isImageFile(previewFile.file) ? (
                ...
                <div className="flex flex-col items-center justify-center min-h-[300px] sm:min-h-[400px] p-4 sm:p-8">
                  <FileIcon className="w-16 h-16 sm:w-24 sm:h-24 text-gray-400 mb-3 sm:mb-4" />
                  <p className="text-sm sm:text-base md:text-lg font-medium text-gray-900 mb-2 text-center">
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <Navbar />
        <div className={`transition-all duration-300 ${isCollapsed && !isHovered ? 'md:ml-20' : 'md:ml-64'} ml-0 pt-16 pb-20 md:pb-0 max-w-full overflow-x-hidden`}>
              ...
              <form onSubmit={handleSubmit} className="p-3 sm:p-4 md:p-6 space-y-3 sm:space-y-4 md:space-y-6">
                ...
                <div>
                  ...
                  <select
                    id="measurement_type"
                    required
                    className={`w-full px-3 py-2.5 sm:py-2 text-base sm:text-sm border-2 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 transition-all duration-200 font-semibold ${measurementType === ''
                      ...
                      <div className="absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-auto">
                        {filteredParties.map((party) => (
                          <div
                            ...
                            className={`px-4 py-2 cursor-pointer hover:bg-blue-50 transition-colors ${selectedPartyId === party.id ? 'bg-blue-100' : ''
                              }`}
                          >
                ...
                <div>
                  ...
                  <div className="relative">
                    <div className="hidden md:block overflow-x-auto shadow-sm border border-gray-200 rounded-lg" style={{ WebkitOverflowScrolling: 'touch' }}>
                            ...
                            <thead className="bg-gray-50 sticky top-0 z-10">
                              <tr>
                                {fields.map((field) => (
                                  <th
                                    key={field}
                                    className="px-2 sm:px-3 py-2 text-left text-xs font-medium text-gray-700 border border-gray-300 whitespace-nowrap bg-gray-50"
                                  >
                                ...
                                <th className="px-2 sm:px-3 py-2 text-left text-xs font-medium text-gray-700 border border-gray-300 whitespace-nowrap bg-gray-50 sticky right-0 z-20">
                                  Actions
                                </th>
                              </tr>
                            </thead>
                                      ...
                                      {field === 'sr_no' ? (
                                        ...
                                        item[field] === 'custom' || (item[field] && !AREA_OPTIONS.includes(item[field])) ? (
                                          ...
                                          <select
                                            data-row-index={index}
                                            data-field={field}
                                            className="w-full px-1 sm:px-2 py-1 text-xs sm:text-sm border-0 focus:outline-none focus:ring-1 focus:ring-blue-500 bg-white min-w-[80px]"
                                            value={item[field] || ''}
                                            onChange={(e) => {
                                              const newItems = [...items];
                                        ...
                                        <input
                                          ...
                                          data-field={field}
                                          readOnly
                                          className="w-full px-1 sm:px-2 py-1 text-xs sm:text-sm border-0 bg-gray-50 text-gray-700 cursor-not-allowed min-w-[60px]"
                                          value={item[field] || ''}
                                        ...
                                        />
                                      ) : (field === 'ro_width' || field === 'ro_height') && (measurementType === 'shutter_sample' || measurementType === 'regular_shutter') ? (
                                        <input
                                          type="text"
                                          data-row-index={index}
                                          data-field={field}
                                          ...
                                          value={item[field] || ''}
                                          placeholder={getFieldPlaceholder(field, measurementType)}
                                          title={`${getFieldPlaceholder(field, measurementType)} (Auto-calculated from ${field === 'ro_width' ? 'Act Width' : 'Act Height'} using round-up rule)`}
                                        />
                                      ) : (
                                        <input
                                          type="text"
                                          data-row-index={index}
                                          data-field={field}
                                          className="w-full px-1 sm:px-2 py-1 text-xs sm:text-sm border-0 focus:outline-none focus:ring-1 focus:ring-blue-500 min-w-[60px]"
                                          value={item[field] || ''}
                    ...
                    <div className="md:hidden space-y-4 mt-4">
                      {items.map((item, index) => (
                        <div key={index} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
                          <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-100">
                            <span className="font-bold text-gray-900 border-l-4 border-blue-500 pl-2">Item {index + 1}</span>
                            <button
                              type="button"
                              onClick={() => handleRemoveClick(index)}
                              disabled={items.length === 1}
                          ...
                          <div className="grid grid-cols-1 gap-4">
                            {fields.map((field) => (
                              <div key={field}>
                                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">{getFieldLabel(field, measurementType)}</label>
                                <div className="relative">
                                  {field === 'sr_no' ? (
                                    <div className="text-sm font-medium text-blue-600 bg-blue-50 px-3 py-2 rounded-lg border border-blue-100 inline-block">
                                      {item.sr_no || '-'}
                                    </div>
                                  ) : (field === 'location_of_fitting' && (measurementType === 'frame_sample' || measurementType === 'regular_frame')) ||
                                    (field === 'location' && (measurementType === 'shutter_sample' || measurementType === 'regular_shutter')) ? (
                                  ...
                                  ) : (field === 'act_width' || field === 'act_height' || field === 'ro_width' || field === 'ro_height') && (measurementType === 'shutter_sample' || measurementType === 'regular_shutter') ? (
                                    <input
                                      type="text"
                                      readOnly
                      ...
                      <div className="flex justify-center pt-4">
                        <button
                          type="button"
                          onClick={addItem}
                          className="flex items-center justify-center w-full px-4 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium shadow-sm active:scale-[0.98] transition-transform"
                        >
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (hasUnsavedChanges()) {
                        setShowConfirmDialog(true);
                        setPendingNavigation(() => () => navigate('/measurement-captain/measurements'));
                      } else {
                        navigate('/measurement-captain/measurements');
                      }
                    }}
