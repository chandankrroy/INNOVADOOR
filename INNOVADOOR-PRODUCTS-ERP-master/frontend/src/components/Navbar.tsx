...
import { User as UserIcon, Undo2, Redo2 } from 'lucide-react';
...
export default function Navbar() {
  ...
  // Check if we're on a measurement create page
  const isMeasurementCreatePage = location.pathname.includes('/measurements/create') || 
                                   location.pathname.includes('/measurement-captain/measurements/create');

  if (!['production_manager', 'site_supervisor', 'measurement_captain'].includes(currentUser?.role || '')) {
    return null;
  }
          ...
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-xl font-semibold text-gray-900">
              {currentUser?.role === 'site_supervisor' 
                ? 'Site Supervisor Management System'
                : currentUser?.role === 'measurement_captain'
                ? 'Measurement Captain System'
                : 'Production Documentation Management System'}
            </h1>
          </div>
          <div className="flex items-center justify-end flex-1 mr-4 gap-3">
            {/* Undo/Redo Buttons - Only show on measurement create pages */}
            {isMeasurementCreatePage && <UndoRedoButtons />}
