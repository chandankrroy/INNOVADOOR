...
export default function ProductionSchedulerSidebar() {
  ...
  // Only show production scheduler menu for production_scheduler role
  if (currentUser?.role !== 'production_scheduler') {
    return null;
  }

  const menuItems = [
    { path: '/scheduler/dashboard', label: 'Dashboard', icon: LayoutDashboard },
    { path: '/scheduler/schedule', label: 'Schedule Production', icon: Calendar },
    { path: '/scheduler/view', label: 'View Scheduled Production', icon: Eye },
    { path: '/scheduler/maintain', label: 'Maintain Scheduled Production', icon: Settings },
